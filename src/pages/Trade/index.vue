  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="(item,index) in address" :key="item.id">
        <span class="username" :class="{selected:item.isDefault==1}" @click="changeDefault(item)">{{
            item.consignee
          }}</span>
        <p>
      ...
      <div class="detail">
        <h5>商品清单</h5>
        <ul class="list clearFix" v-for="detail in trade.detailArrayList" :key="detail.skuId">
          <li>
...
export default {
  ...
  methods: {
    ...
    async submitOrder() {
      ...
      let result = await this.$api.reqOrder(tradeNo, data)
      if (result.code == 200) {
