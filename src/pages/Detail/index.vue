            ...
            <div class="chooseArea">
              ...
              <dl v-for="spuSaleAttr in spuSaleAttrList" :key="spuSaleAttr.id">
                ...
                <dd v-for="spuSaleAttrValue in spuSaleAttr.spuSaleAttrValueList" :key="spuSaleAttrValue.id"
                    ...
                    @click="changeChecked(spuSaleAttr.id,spuSaleAttrValue.id)">
...
export default {
  ...
  methods: {
    changeChecked(outid, inid) {
      this.spuSaleAttrList.forEach((outitem) => {
        if (outitem.id == outid) {
          outitem.spuSaleAttrValueList.forEach((initem) => {
            if (initem.id == inid) {
              initem.isChecked = 1
            } else {
              initem.isChecked = 0
  ...
  watch: {
    skuNum(newVal, oldVal) {
      let val = newVal * 1
      if (isNaN(val) || val < 1) {
        this.skuNum = parseInt(oldVal)
      } else {
